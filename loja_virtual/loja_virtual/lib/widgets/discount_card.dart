              ...
              onFieldSubmitted: (text) {
                Firestore.instance.collection("coupons").document(text).get().then((docSnap) {
                  if (docSnap.data != null) {
                    CartModel.of(context).setCoupon(text,double.parse(docSnap.data["percent"].toString()));
                    ...
                    Scaffold.of(context).showSnackBar(
                        SnackBar(content: Text("Desconto não existente"), backgroundColor: Colors.redAccent,)
                    );
                  }
                });
              },
